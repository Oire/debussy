...
- **`check-git-guard`** (cross-platform) — blocks git writes that publish or
  destroy work: `push`, `commit --amend`, `rebase`, `reset --hard`, `restore`,
  destructive `checkout`, `clean -f`, bulk staging (`add -A`, `commit -a`), and
  `git mv`/`git rm`. Staging named paths and committing locally are allowed.
